...
class Controller:
    ...
    def _warm_up(self):
        if self.device == 'cuda:0':
            obs = torch.ones((1,45)).to(self.device)
        else:
            obs = torch.ones((1,45))
        for _ in range(10):
            _ = self.policy(obs)
...
if __name__ == "__main__":
    ...
    # controller.replay_buffer.plot_ang_vel()
    # controller.replay_buffer.plot_gravity()
    # controller.replay_buffer.plot_dof_pos()
    # controller.replay_buffer.plot_dof_vel()
    # controller.replay_buffer.plot_actions()
