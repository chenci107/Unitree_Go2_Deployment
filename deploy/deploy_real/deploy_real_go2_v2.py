...
class Go2Robot:
    def __init__(self,config:Config,net) -> None:
        self.config = config
        self.net = net
    ...
    def _start_robot(self):
        ...
        self.lowcmd_publisher = ChannelPublisher(self.config.lowcmd_topic,LowCmdGo)
    ...
    def _compute_observations(self):
        ...
        # print('The shape of ang_vel is:',ang_vel.shape)
        # print('The shape of gravity_orientation is:',gravity_orientation.shape)
        # print('The shape of self.cmd is:',self.cmd.shape)
        # print('The shape of qj_obs is:',qj_obs.shape)
        # print('The shape of dqj_obs is:',dqj_obs.shape)
        # print('The shape of self.action is:',self.action.shape)

        self.obs[:3] = ang_vel    # (1,3)
        self.obs[3:6] = gravity_orientation  # (3,)
        self.obs[6:9] = self.cmd * self.config.command_scale # (3,)
        self.obs[9:21] = qj_obs   # (12,)
        self.obs[21:33] = dqj_obs # (12,)
        self.obs[33:45] = self.action # (12,)

    def _apply_actions(self,actions):
        target_dof_pos = self.config.default_angles + actions * self.config.action_scale
        for i in range(12):
            motor_idx = self.config.joint2motor_idx[i]
...
class InferAction():
    def __init__(self,config,net):
        ...
        self._warm_up()
        self.real_env = Go2Robot(config=self.config,net=self.net)
        self.max_path_length = 200


    def _warm_up(self):
        obs = torch.ones((1,45),device=self.device)
        for i in range(10):
            _ = self.policy(obs)
        print('Network has been warmed up.')

    def run(self):
        obs_ = self.real_env.reset()
        # obs = torch.from_numpy(np.expand_dims(obs_,axis=0)).to(self.device)
        obs = torch.from_numpy(obs_).unsqueeze(0).to(self.device)
        for _ in range(self.max_path_length):
            try:
                action_ = self.policy(obs)
                action = action_.squeeze(0).cpu().detach().numpy()
                # action = action_.detach().numpy().squeeze()
                obs_,_,_,_ = self.real_env.step(action)
                obs = torch.from_numpy(np.expand_dims(obs_,axis=0)).to(self.device)
                # obs = torch.from_numpy(obs_).unsqueeze(0)
                if self.real_env.remote_controller.button[KeyMap.select] == 1:
                    break
            except KeyboardInterrupt:
                break

    def zero_torque_state(self):
        self.real_env.zero_torque_state()

    def move_to_default_pos(self):
        self.real_env.move_to_default_pos()

    def default_pos_state(self):
        self.real_env.default_pos_state()

    def damping_cmd(self):
        self.real_env.damping_cmd()
        print('Exit')
...
if __name__ == "__main__":
    ...
    # Load config
    config_path = "/home/unitree/code/unitree_rl_gym_0528/deploy/deploy_real/configs/go2.yaml"
    config = Config(config_path)

    runner = InferAction(config=config,net=args.net)

    runner.zero_torque_state()
    runner.move_to_default_pos()
    runner.default_pos_state()
